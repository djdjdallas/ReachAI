            ...
            <div className="bg-white rounded-[2.5rem] p-10 soft-shadow border border-stone-100 space-y-8">
              <div className="space-y-6">
                ...
                <div className="flex items-start gap-5">
                  ...
                  <div>
                    <h4 className="font-bold text-stone-900">
                      4. Enable &ldquo;Connected Tools&rdquo; in Instagram
                    </h4>
                    <p className="text-sm text-stone-500">
                      Required so Meta forwards new DMs to Clinchd. One-time
                      setting, takes 10 seconds.
                    </p>
                  </div>
                </div>
              </div>

              {/* Connected Tools acknowledgment */}
              <div className="rounded-3xl border-2 border-amber-200 bg-amber-50/60 p-6 space-y-4">
                <div className="flex items-start gap-3">
                  <AlertTriangle className="w-5 h-5 text-amber-600 mt-0.5 shrink-0" />
                  <div className="flex-1">
                    <h4 className="text-sm font-black text-stone-900 uppercase tracking-widest">
                      Action Required: Turn On Connected Tools
                    </h4>
                    <p className="text-xs text-stone-600 mt-1 leading-relaxed">
                      Without this setting, Instagram will block Clinchd from
                      receiving DMs from new leads. You only need to do this
                      once.
                    </p>
                  </div>
                </div>

                <ol className="text-xs text-stone-700 space-y-1.5 pl-8 list-decimal marker:font-bold marker:text-amber-700">
                  ...
                  <li>Tap <strong>Connected tools</strong></li>
                  <li>
                    Turn on <strong>&ldquo;Allow access to messages&rdquo;</strong>
                  </li>
                </ol>

                <label className="flex items-start gap-3 cursor-pointer pt-2 border-t border-amber-200/60">
                  <span className="relative flex items-center mt-0.5">
                    <input
                      type="checkbox"
                      className="sr-only peer"
                      checked={connectedToolsAcknowledged}
                      onChange={(e) =>
                        setConnectedToolsAcknowledged(e.target.checked)
                      }
                    />
                    <span className="w-5 h-5 bg-white border-2 border-stone-300 rounded-md peer-checked:bg-[#ff7e67] peer-checked:border-[#ff7e67] transition-all flex items-center justify-center">
                      <Check className="text-white w-3 h-3 opacity-0 peer-checked:opacity-100 transition-opacity" />
                    </span>
                  </span>
                  <span className="text-xs font-bold text-stone-700 leading-relaxed">
                    I&apos;ve turned on &ldquo;Allow access to messages&rdquo;
                    in the Instagram app&apos;s Connected Tools settings.
                  </span>
                </label>
              </div>
              ...
              <div className="pt-4">
                <a
                  href={canConnect ? "/api/auth/instagram" : undefined}
                  onClick={(e) => {
                    if (!canConnect) e.preventDefault();
                  }}
                  aria-disabled={!canConnect}
                  className={`w-full py-5 rounded-[1.25rem] text-xl font-black shadow-2xl transition-all flex items-center justify-center gap-3 ${
                    canConnect
                      ? "bg-[#ff7e67] text-white shadow-[#ff7e67]/20 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
